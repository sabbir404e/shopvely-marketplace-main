...
-- Helper function to generate truly unique referral codes
...
-- Prevent referral codes from ever being changed (Immutability)
...
-- Trigger to prevent referral code change
do $$ begin
  if not exists (select 1 from pg_trigger where tgname = 'check_referral_code_immutable') then
